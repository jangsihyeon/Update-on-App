...
:  
   ...
   y 값이 각각의 max, min 값에 맞춰서 움직이게 
    ...
    ==> y 값의 스케일을 각각 정해서 fully border로 보기  -> set_ylim으로 가능할듯 
...
# 그래프 업데이트 
    def update_scope(self, data):
        ...
        if self.vel_order_check.isChecked():
            self.par1.plot(self.y_data_O_Vel, label="O_Vel", color='black', linestyle='--')

        if self.vel_real_check.isChecked():
            self.par1.plot(self.y_data_R_Vel, label="R_Vel", color='purple')

        if self.cur_order_check.isChecked():
            self.par2.plot(self.y_data_O_Cur, label="O_Cur", color='green', linestyle='--')
